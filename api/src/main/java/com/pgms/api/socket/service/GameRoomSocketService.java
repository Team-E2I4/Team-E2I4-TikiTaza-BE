...
import com.pgms.api.socket.exception.SocketException;
...
public class GameRoomSocketService {
	...
	public GameRoomEnterResponse enterGameRoom(Long roomId, Long memberId) {
		...
		if (gameRoom.isFull()) {
			throw new SocketException(GameRoomErrorCode.GAME_ROOM_FULL);
		}
	...
	private Member getMember(Long memberId) {
		return memberRepository.findById(memberId)
			.orElseThrow(() -> new SocketException(MemberErrorCode.MEMBER_NOT_FOUND));
	}

	private GameRoom getGameRoom(Long roomId) {
		return gameRoomRepository.findById(roomId)
			.orElseThrow(() -> new SocketException(GameRoomErrorCode.GAME_ROOM_NOT_FOUND));
	}

	private void validateIsMatchedGameRoom(Member member, Long roomId) {
		if (!member.getGameRoom().getId().equals(roomId)) {
			throw new SocketException(GameRoomErrorCode.GAME_ROOM_MISMATCH);
		}
	}
